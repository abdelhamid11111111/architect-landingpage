...
function StatItem({ stat, Icon }: { stat: Stat; Icon: (typeof icons)[number] }) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-start gap-4 py-8 border-t border-ink/10">
      <Icon className="text-3xl text-bronze" />
      <p className="font-display text-5xl lg:text-6xl text-ink">
        {value}
        {stat.suffix}
      </p>
      <p className="font-sans text-sm tracking-[0.1em] uppercase text-ink/50">{stat.label}</p>
    </div>
  );
}
    ...
    <section className="relative bg-offwhite py-28 lg:py-36">
      <div className="container-lux grid lg:grid-cols-12 gap-12 lg:gap-8">
        <div className="lg:col-span-5">
          <p className="eyebrow mb-5">Pourquoi Atelier Verrier</p>
          <h2 ref={titleRef} className="font-display text-4xl sm:text-5xl leading-[1.05] mb-6">
            Une exigence portée depuis près de deux décennies
          </h2>
          <p className="font-sans text-ink/55 leading-relaxed max-w-md">
        ...
        <div className="lg:col-span-7 grid grid-cols-2 gap-x-8">
          {stats.map((stat, i) => (
